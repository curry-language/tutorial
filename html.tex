...
in the library \ccode{HTML}\index{library!HTML}
...
The \code{HTML} library defines also a wrapper function
...
To support the creation of dynamic HTML documents, the \code{HTML} library
...
\end{prog}
(\code{makecurrycgi}\pindex{makecurrycgi} is a shell script
stored in the \ccode{bin} directory of \pakcs).
...
\begin{prog}
makecurrycgi -o \char126/cgi-bin/myscript.cgi myscript
\end{prog}
...
CGI since the \code{HTML} library defines an abstraction layer
...
The \code{HTML} library contains definitions of various input
...
For this purpose, the \code{HTML} library defines
...
For this purpose, the \code{HTML} library defines the type of event handlers as
\begin{prog}
...
Thus, the \code{HTML} library contains the following type definition
...
(\code{readInt}\pindex{readInt} is defined in the standard library
\code{Read} and converts a string into a number)
...
(\code{doesFileExist}\pindex{doesFileExist} is an action defined
in the library \code{Directory} that checks the existence of a file):
\begin{prog}
...
characters; \code{readNat}\pindex{readNat} is defined in the
standard library \code{Read} and interprets a string as a natural number):
\begin{prog}
readVoteFile :: IO [Int]
readVoteFile = do
...
\end{prog}
Similarly, \code{writeVoteFile} is an action that write a list of
numbers into the vote file. Similarly to the definition of
\code{writeVisitFile} in Section~\ref{sec-writeVisitFile},
...
\end{prog}
Using \code{writeVoteFile}, we define an action \code{initVoteFile}
...
you can generate the CGI program with \ccode{makecurrycgi}\pindex{makecurrycgi}
with the option \ccode{-noerror}.
...
of the \code{HTML} library in Curry.
If the web script fails, i.e., the execution produces the message
\ccode{No more solutions}, compile it again with
the option \ccode{-debug} as described above.
If the new execution shows a failure of an expression
like \ccode{'1'=:='2'} resulting in the subsequent failure of an
equational constraint like \code{("FIELD_1" =:= $\ldots$)},
...
\ccode{CookieParam} in the \code{HTML} library) which we will not describe
...
The \code{HTML} library provides two functions to set
...
The \code{HTML} library provides the functions
\ccode{urlencoded2string} and \ccode{string2urlencoded}
...
\end{prog}
The I/O action \ccode{getDirectoryContents}\pindex{getDirectoryContents}
is defined in the system library \code{Directory}
and returns the list of all entries in a directory.
The function \ccode{entry2html} checks for an entry whether it
...
(\ccode{doesDirectoryExist}\pindex{doesDirectoryExist}
...
\end{prog}
Finally, the prelude function \ccode{mapIO}\pindex{mapIO} applies
