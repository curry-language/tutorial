...
\begin{itemize}
\item{} 
A \emph{function}\index{function} defines
...
\item{} 
A \emph{data structure}\index{data structure} is a way to organize data.
...
%
\begin{quote}
An \emph{expression}\index{expression!definition}
...
\pindex{String}\index{List}\pindex{Success}\index{unit type}\pindex{()}
%
...
The type \emph{Success}\pindex{Success} has no visible literal values
...
\emph{constraints}\index{constraint}.
Usually, they occur in conditions and are checked for satisfiability.
The symbol \emph{success}\pindex{success} is a predefined
...
\emph{fails} and neither \ccode{True} nor \ccode{False} is returned.
\\[1ex]
...
applied to expressions $u$ and $v$, i.e.,
$u\;\mbox{\tt \&\&}\;v$, returns \ccode{True}
if and only if $u$ and $v$ can be evaluated to \ccode{True}.
\\[1ex]
The \emph{Boolean disjunction}\index{Boolean disjunction}%
\index{disjunction!Boolean}
applied to expressions $u$ and $v$, i.e.,
$u\;\mbox{\tt ||}\;v$, returns \ccode{True}
if and only if $u$ or $v$ can be evaluated to \ccode{True}.
\\[1ex]
...
%
\begin{prog}
\char92\progit{args} -> \progit{left-hand side}
\end{prog}
%
...
Functions can be \emph{non-deterministic}\index{function!non-deterministic}.
...
In a \emph{functional pattern}\index{functional pattern}
...
\noindent
Code employing functional patters should be preferred to
...
required constrain are $[3,6]$ and $[2,1,4]$.
...
%
\begin{prog}
sortClasses l = sort (\char92(x,y) (u,v) -> x<u || x==u \&\& ord y <= ord v) l
\end{prog}
%
...
Extra variables must be explicitly declared \ccode{free}
...
The constrained equality returns the type \emph{Success}\index{Success}.
This type has no visible constructors.
An expression of type \emph{Success} either succeeds or fail.
There exists predefined operations, \ccode{success} and
\ccode{failed}, to encode successes and failures in a
program.
...
%
\begin{prog}
Prelude> \userinput{x ++ [3,4] =:= [1,2,3,4]  where x free}
\{x=[1,2]\} success
\end{prog}
%
...
The first condition of the operation \ccode{parseTerm}
...
by using the function \code{findall}.

\begin{exercise}
Define an I/O action \code{filelength} that reads requests the name
